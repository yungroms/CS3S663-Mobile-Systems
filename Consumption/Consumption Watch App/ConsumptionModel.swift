...
class ConsumptionModel: ObservableObject {
    ...
    func resetDailyIfNeeded() {
        ...
        if today != lastResetDay {
            // Before resetting, create a DailySummary record for the day that just ended.
            let context = PersistenceController.shared.container.viewContext
            let dailySummary = DailySummary(context: context)
            dailySummary.date = lastResetDay  // Use the previous day's start
            dailySummary.calories = Int16(dailyCalories)
            dailySummary.water = Int16(dailyWater)
            
            do {
                try context.save()
                print("Saved daily summary for \(lastResetDay). Calories: \(dailyCalories), Water: \(dailyWater)")
            } catch {
                print("Error saving daily summary: \(error.localizedDescription)")
            }
            
            // Optionally, update in-memory yesterday values if needed immediately.
            ...
            print("Daily values reset. Yesterday's totals stored.")
        }
    }
    ...
    func getYesterdayTotals() -> (calories: Int, water: Int) {
        let context = PersistenceController.shared.container.viewContext
        ...
        request.predicate = NSPredicate(format: "date >= %@ AND date < %@", startOfYesterday as NSDate, endOfYesterday as NSDate)
        ...
        do {
            ...
            if let summary = results.first {
                return (calories: Int(summary.calories), water: Int(summary.water))
            }
        } catch {
            print("Error fetching yesterday's summary: \(error.localizedDescription)")
        }
        return (calories: 0, water: 0)
    }
}
